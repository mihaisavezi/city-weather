...
export function CityTable({ cities, showWeather = false, onEdit, onDelete }: CityTableProps) {
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {cities.map((city) => (
            <tr key={city.id}>
              ...
              <td className="table-cell">{city.estimatedPopulation.toLocaleString()}</td>
              
              {showWeather && isSearchResult(city) && (
                <>
                  <td className="table-cell">
                    <div className="text-sm text-gray-900">{city.weather.temperature}°C</div>
                    <div className="text-sm text-gray-500">{city.weather.description}</div>
                  </td>
                  <td className="table-cell">
